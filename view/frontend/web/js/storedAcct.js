require(["jquery"], function ($) {
    $(document).ready(function () {
        ...
        $('#stored_acct').change(function () {
            if ($(this).val() == '') {
                if ($('#expiration_month').get(0)) {
$('#expiration_month').get(0).selectedIndex = 0; }
                if ($('#expiration_year').get(0)) {
$('#expiration_year').get(0).selectedIndex = 0; }
                if ($('#cc_number')) {
$('#cc_number').val(''); }
                if ($('#echeck_acct_type').get(0)) {
$('#echeck_acct_type').get(0).selectedIndex = 0; }
                if ($('#echeck_acct_number')) {
$('#echeck_acct_number').val(''); }
                if ($('#echeck_routing_number')) {
...
$('#cc_number').val(''); }
        ...
        $('#submitBtn').click(function () {
            if ($('#payment_type') && $('#payment_type').prop("disabled", true)) {
$('#payment_type').prop("disabled", false); }
            $.ajax({
